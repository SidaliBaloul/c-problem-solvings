...
string Replace(string word, string replace, string newe)
{
	short pos = 0;

	while ((pos = word.find(replace)) != std::string::npos)
	{
		word = word.replace(pos, replace.length(), newe);
...
string DateFormat(sDate date, string format )
{
	string thenewword = "";

	thenewword = Replace(format, "dd", to_string(date.day));
	thenewword = Replace(thenewword, "mm", to_string(date.month));
	thenewword = Replace(thenewword, "yyyy", to_string(date.year));
...
int main()
{
	...
	cout << "\n" << DateFormat(date, "dd-mm-yyyy") << "\n";
	cout << "\n" << DateFormat(date, "day : dd, month :  mm , year : yyyy") << "\n";
